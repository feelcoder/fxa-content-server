...
define(function (require, exports, module) {
  ...
  var View = SignInView.extend({
    ...
    beforeRender: function () {
      ...
      if (relier.has('uid')) {
        // relier specified a uid, if the uid is no longer registered
        // then the user can only signin/signup if the relier (via the broker)
        // supports uid changes.
        return p.all([
          this.user.checkAccountEmailExists(account),
          this.user.checkAccountUidExists(account)
        ]).spread(function (emailExists, uidExists) {
          /*
           * uidExists - false, emailExists - false - let user sign up w/ email
           * uidExists - true, emailExists - false - uid exists but
           *   doesn't match email, how'd this happen? let the user sign up
           * uidExists - false, emailExists - true - sign in w/ new uid
           * uidExists - true, emailExists - true - assume for the same
           *   account, try to sign in
           */
          if (! emailExists) {
